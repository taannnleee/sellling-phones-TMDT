...
function phanTich_URL_chiTietSanPham() {
    ...
    var s = addThongSo('Màn hình', sanPhamHienTai.screen);
    s += addThongSo('Hệ điều hành', sanPhamHienTai.os);
    s += addThongSo('Camara sau', sanPhamHienTai.camara);
    s += addThongSo('Camara trước', sanPhamHienTai.camaraFront);
    s += addThongSo('CPU', sanPhamHienTai.cpu);
    s += addThongSo('RAM', sanPhamHienTai.ram);
    s += addThongSo('Bộ nhớ trong', sanPhamHienTai.rom);
    s += addThongSo('Thẻ nhớ', sanPhamHienTai.microUSB);
    s += addThongSo('Dung lượng pin', sanPhamHienTai.battery);
    info.innerHTML = s;

    // Cập nhật hình
    var hinh = divChiTiet.getElementsByClassName('picture')[0];
    hinh = hinh.getElementsByTagName('img')[0];
    hinh.src = sanPhamHienTai.imageUrl;
    document.getElementById('bigimg').src = sanPhamHienTai.imageUrl;
