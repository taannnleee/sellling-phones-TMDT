window.onload = function () {
	...
	for (var t of tags) addTags(t, "index?search=" + t);
	...
	if (filters.length) { // có filter
		...
		addKhungSanPham('Samsung', yellow_red, ['category=Samsung', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('Apple', blue, ['category=Apple', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('Xiaomi', yellow_red, ['category=Xiaomi', 'sort=rateCount-decrease'], soLuong, div);
		addKhungSanPham('Oppo', green, ['category=Oppo', 'sort=rateCount-decrease'], soLuong, div);
...
function timKiemTheoTen(ten) {
	$.ajax({
		type: "GET",
		url: "/index/timKiemTheoTen",
		data: { ten: ten },
		async: false ,// Đặt Async thành false
		success: function(response) {
			listSearchProduct = response;
	...
	for (var i = 0; i < filters.length; i++) {
		...
		switch (dauBang[0]) {
			case 'search':
				...
				timKiemTheoTen(dauBang[1])
				result=listSearchProduct;
				console.log(listSearchProduct)
			...
			case 'price':
				...
				timKiemTheoGiaTien( prices[0], prices[1]);
				result= listSearchPriceProduct;
				console.log(listSearchPriceProduct)
				break;

			case 'category':
				timKiemTheoCongTySanXuat(dauBang[1]);
				console.log(dauBang[1])
				result=listSearchCategoryProduct;
				console.log(listSearchCategoryProduct)
				if (saveFilter) filtersFromUrl.category = dauBang[1];
				break;
...
function timKiemTheoCongTySanXuat(tenCongTy){
	$.ajax({
		type: "GET",
		url: "/index/timKiemTheoCongTySanXuat",
		data: { tenCongTy: tenCongTy},
		async: false ,// Đặt Async thành false
		success: function(response) {
			console.log(tenCongTy)
			listSearchCategoryProduct = response;
			console.log(listSearchCategoryProduct)
		},
		error: function(xhr, status, error) {
			console.error("Error:", error);
...
function timKiemTheoGiaTien(giaMin,giaMax)
{
	$.ajax({
		type: "GET",
		url: "/index/timKiemTheoGiaTien",
		data: { giaMin: giaMin,giaMax:giaMax },
		async: false ,// Đặt Async thành false
		success: function(response) {
			listSearchPriceProduct = response;
		},
		error: function(xhr, status, error) {
			console.error("Error:", error);
		}
...
function addAllChoosedFilter() {
	...
	if (filtersFromUrl.category != '')
		addChoosedFilter('category', filtersFromUrl.category);
...
function addCompany(img, nameCompany) {
	var link = createLinkFilter('add', 'category', nameCompany);
